...
\geometry{margin=2.5cm}          % Sets all margins to 2.5 cm
% Colors
\usepackage{xcolor}
% For custom lists
\usepackage{enumitem}
...
%\setlist[itemize]{label={\symbolfont✦}, itemsep=3pt, topsep=3pt}

\usepackage{comment}    % to i can write a paragraphe of comments 
%\usepackage{enumitem}   % to items like →


% Apply ✦ globally to all itemize lists
\setlist[itemize]{label={\symbolfont}, itemsep=6pt, topsep=6pt}
% Font management (required for XeLaTeX)
%\usepackage{fontspec}
% Symbol font
%\newfontfamily{\symbolfont}{DejaVu Sans}   % for the star ✦
...
\usepackage{float}               % Enables precise float placement using [H]

\usepackage{booktabs}            % Professional-quality tables (toprule, midrule, bottomrule)
\usepackage{array}               % Extended column formatting options
\usepackage[table]{xcolor}       % Enables colors inside tables
\usepackage{colortbl}            % Additional color support for tables

% --- Custom subtle academic colors ---
\definecolor{headercolor}{RGB}{230,240,250}   % Light blue for table header
\definecolor{rowcolorlight}{RGB}{248,250,252} % Very light gray for alternating rows

\usepackage{fontspec}            % Allows system font selection (XeLaTeX/LuaLaTeX only)
\usepackage{unicode-math}        % Enables modern math font handling

\setmainfont{TeX Gyre Termes}    % Sets main document font (similar to Times)
\setmathfont{TeX Gyre Termes Math} % Sets math font

\usepackage{polyglossia}         % Modern multilingual support for XeLaTeX

\setmainlanguage{english}        % Main document language
\setotherlanguage{arabic}        % Secondary language (Arabic support)
\hyphenpenalty=10000             % Prevents word hyphenation
\exhyphenpenalty=10000           % Prevents explicit hyphen breaks


% ===== Elegant marked lists =====
\usepackage{enumitem}
\usepackage{xcolor}
% Colors
...
\chapter*{Summary}
\section{Summary}
This project involves the design and development of a mobile application called Allo Artisan, aimed at connecting clients with artisans in Algeria. Following an analysis of existing solutions and their limitations, we proposed a solution built around three main axes: trust, functionality, and accessibility. The application provides key features such as QR code identity verification, diploma validation, a multi-criteria search system, an availability calendar, a post and notification mechanism for service requests. Technically, the application was developed using Flutter for the mobile interface, Go (Golang) for the backend, and PostgreSQL as the database management system, deployed via Docker.
Keywords : Mobile application, client-artisan connection, Flutter, Go, PostgreSQL

 \section{ملخص} 
 Allo Artisan يتمحور هذا المشروع حول تصميم وتطوير تطبيق هاتف يحمل اسم يهدف إلى تسهيل التواصل بين الزبائن والحرفيين في الجزائر. بعد تحليل التطبيقات الموجودة في نفس المجال ونقاط ضعفها، اقترحنا حلاً مبنياً على ثلاثة محاور رئيسية هي: الثقة، الوظائف، .
يوفر التطبيق مجموعة من الميزات الأساسية أبرزها: التحقق من هوية الحرفي عن طريق رمز الاستجابة السريعة، والتحقق من الشهادات والوثائق المهنية، ونظام بحث متعدد المعايير، وتقويم لعرض أوقات التوفر، إضافةً إلى نظام منشورات وإشعارات خاصة بطلبات الخدمة العاجلة والعادية.أما على الصعيد التقني، فقد تم تطوير التطبيق باستخدام Flutter لتصميم الواجهات، وGo (Golang) لتطوير الخادم وبناء واجهة برمجية من نوع REST، وPostgreSQL كنظام لإدارة قواعد البيانات، مع اعتماد Docker لضمان نشر مستقر ومحمول.
الكلمات المفتاحية : تطبيق جوال، ربط الزبائن بالحرفيين، بحث متعدد المعايير، Flutter، Go، PostgreSQL.
...
\begin{center}
    \includegraphics[width=2.5cm]{khedmni .png} 
    \par
    \textbf{Khedamni Logo}
\end{center}
...
\begin{center}
    \includegraphics[width=2.5cm]{d9d9.png} 
    \par
    \textbf{De9de9 Logo}
\end{center}
...
\begin{center}
    \includegraphics[width=2.5cm]{afriha.png} 
    \par
    \textbf{Afriha Logo}
\end{center}
...
\end{itemize}
%\begin{center}
 %   \includegraphics[width=0.6\textwidth]{pillars_final.png}
%\end{center}
%\begin{figure}[H]
    %\centering
    %\includegraphics[width=0.8\textwidth]{photo_2026-05-18_22-23-55.jpg}
    %\caption{What is our application based on?} 
     % \label{fig:What is our application based on?}
%\end{figure} 
\section{Conclusion}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{use case 7.drawio.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{sequance-client_request_service.drawio.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{sequance-auth.drawio (1).png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{sequance-create_account_artisan.drawio (1).png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{activity.drawio.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{class diagram1.jpg}
...
\hfill
    ...
    \centering
    \includegraphics[width=2.5cm]{go langage.png}
...
\begin{minipage}[c]{0.3\textwidth}
    \centering
    \includegraphics[width=2.5cm]{vs code.jpg}
...
\begin{minipage}[c]{0.3\textwidth}
    \centering
    \includegraphics[width=2.5cm]{Android studio.png}
...
\begin{minipage}[c]{0.3\textwidth}
    \centering
    \includegraphics[width=2.5cm]{doker.png}
...
\begin{minipage}[c]{0.3\textwidth}
    \centering
    \includegraphics[width=2.5cm]{postgresql.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{deployment diagram.jpg}
...
\begin{minipage}[c]{0.38\textwidth}
    \centering
    \includegraphics[width=\linewidth]{login_screen.png}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[c]{0.3\textwidth}
        \includegraphics[width=\textwidth]{register_step1.png}
        \caption{Step 1}
    \end{subfigure}
    \hfill
    \begin{subfigure}[c]{0.3\textwidth}
        \includegraphics[width=\textwidth]{register_step2.png}
        \caption{Step 2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[c]{0.3\textwidth}
        \includegraphics[width=\textwidth]{register_step3.png}
        \caption{Step 3}
    \end{subfigure}
...
\begin{minipage}[c]{0.38\textwidth}
    \centering
    \includegraphics[width=\linewidth]{home_screen_client.png}
...
\begin{figure}[H]
    ...
    \begin{minipage}[c]{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{demand_normal.png}
        \caption{Normal Request}
    \end{minipage}
    \hfill
    \begin{minipage}[c]{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{demand_urgent.png}
...
\newline
$\bullet$ A "QR Code" button to generate his personal QR code \newline
$\bullet$ A "Settings" button to modify his information \newline
$\bullet$ A visibility status that can be toggled to appear or not to clients \newline
$\bullet$ Management of his availabilities via a calendar \newline
$\bullet$ The list of his subscribers \newline
$\bullet$ A"Create a publication"section to share his work with a photo 
\vspace{10pt}

\begin{figure}[h]
    \centering
    \begin{subfigure}[c]{0.3\textwidth}
        \includegraphics[width=\textwidth]{privayte_profile.png}
        \caption{Profile 1}
    \end{subfigure}
    \hfill
    \begin{subfigure}[c]{0.3\textwidth}
        \includegraphics[width=\textwidth]{privat_profile2.png}
        \caption{Profile 2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[c]{0.3\textwidth}
        \includegraphics[width=\textwidth]{private_profile3.png}
        \caption{Profile 3}
    \end{subfigure}
    \caption{Artisan Profile Interface}
    \label{fig:artisan_Private Profile}
\end{figure}
...
and their published work posts that showcase completed services.
 \vspace{10pt}

%\begin{figure}[h]
 %\begin{subfigure}[c]{0.3\textwidth}
  %      \includegraphics[width=\textwidth]%{artisan_public_profil2.png}
  %      \caption{Profile}
   % \end{subfigure}
   % \hfill
    %\begin{subfigure}[c]{0.3\textwidth}
   ...
   % \end{subfigure}
    ...
    %\label{fig:artisan_Public Profile}
%\end{figure}
\section{Conclusion}
